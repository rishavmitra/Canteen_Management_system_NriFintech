...
public class FeedbackExcelGenerator {
    public void generate(List<OrderEntity> orderslist, HttpServletResponse response) throws IOException {
        ...
        String fileName = "Feedbacks_" + dtf.format(now) + ".xlsx";
        ...
        // Setting the Title
        int rowNum=1;
        titleCell.setCellValue("List of the feedbacks - " + dtf.format(now));
        Row r = sheet.createRow(rowNum++);
        r.createCell(0).setCellValue("User Name");
        r.createCell(1).setCellValue("Order Date");
        r.createCell(2).setCellValue("Order Id");
        r.createCell(3).setCellValue("Item Id");
        r.createCell(4).setCellValue("Item Name");
        r.createCell(5).setCellValue("Price");
        r.createCell(6).setCellValue("Feedback");
        
        
        // Creating Rows for the Orders
        rowNum = 3;
        for (OrderEntity order : orderslist) {
            ...
            row.createCell(3).setCellValue(order.getFood().getID());
            row.createCell(4).setCellValue(order.getFood().getName());
            row.createCell(5).setCellValue(order.getTotalPrice());
            row.createCell(6).setCellValue(order.getFeedback());
        }
